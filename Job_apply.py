...
# Configuring CSV
with open("output.csv", 'r') as infile, open("History/all_applied.csv", 'a', newline='') as outfile:
...
def setupLogger() -> None:
    ...
    logging.basicConfig(filename=('./logs/' + str(dt) + '--job_apply.log'), filemode='w',
                        format='%(asctime)s - %(levelname)s - %(message)s', datefmt='./logs/%d-%b-%y %H:%M:%S')
...
class EasyApplyBot:
    ...
    def applications_loop(self, position, location):
        ...
        while time.time() - start_time < self.MAX_SEARCH_TIME:
            try:
                ...
                randoTime: float = random.uniform(2.5, 3.9)
                ...
                for i, jobID in enumerate(jobIDs):
                    ...
                    if count_application != 0 and count_application % 20 == 0:
                        sleepTime: int = random.randint(10, 15)
    ...
    def extract_hiring_team_link(self):
        try:
            hiring_team_link_element = self.browser.find_element(By.XPATH, '//*[@id="main"]/div/div[2]/div/div[2]/div[1]/div/div[2]/div/div[2]/div[2]/a')
            hiring_team_link = hiring_team_link_element.get_attribute('href')
            return hiring_team_link
        except NoSuchElementException:
            log.info("Hiring team link not found on this page.")
            return None
    
    def get_easy_apply_button(self):
        try:
            hiring_team_link = self.extract_hiring_team_link()
            if hiring_team_link:
                log.info("Hiring Team Link: " + hiring_team_link)
...
#top10 applicant &f_EA=true
#monthly sort r604800
    def next_jobs_page(self, position, location, jobs_per_page):
        self.browser.get(
            "https://www.linkedin.com/jobs/search/?f_LF=f_AL&f_E=1%2C2%2C3&f_TPR=r2592000&keywords=" +
